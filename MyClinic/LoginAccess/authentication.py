...
from .models import User, generate_customer_id

def handle_firebase_token(id_token, role='patient', mobile_number=None):
    """
    Verifies the Firebase ID token, retrieves or creates a user, and updates the role if provided.
    """
    try:
        ...
        if not user:
            ...
            user = User.objects.create_user(
                mobile_number=mobile_number or f"{firebase_uid}@example.com",
...
class FirebaseAuthentication(authentication.BaseAuthentication):
    def authenticate(self, request):
        ...
        try:
            ...
            user = handle_firebase_token(token)
            return (user, None)
